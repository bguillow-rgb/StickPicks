---
title: "How to Season a New Humidor: Boveda vs Distilled Water"
description: "Step-by-step seasoning for a new Spanish cedar humidor, Boveda method, distilled-water method, how long it takes, and how to tell when it's done."
tier: cluster
targetQuery: "how to season a humidor"
relatedQueries:
  - "seasoning a new humidor"
  - "humidor seasoning boveda"
  - "humidor seasoning distilled water"
  - "how long does humidor seasoning take"
  - "do you need to season a humidor"
quickAnswer: "A new Spanish cedar humidor must be seasoned before holding cigars or it will pull moisture from the wrappers and dry the collection. The Boveda method places two 84% RH packs inside an empty humidor for 14 days. The distilled-water wipe method takes 7-10 days but requires more attention. Skip seasoning and you will lose the first round of cigars you load."
publishedAt: "2026-04-29"
author: "Bob Guillow"
relatedSlugs:
  - "boveda-vs-distilled-water-humidor"
  - "cigars-power-outage-survival"
  - "how-to-store-cigars"
faqs:
  - q: "Do you really need to season a humidor?"
    a: "Yes for any new Spanish cedar humidor. The wood is kiln-dried and below 50% internal moisture; without seasoning it will absorb moisture from your cigars and dry them out within days."
  - q: "How long does it take to season a humidor?"
    a: "Boveda 84% method: 14 days untouched. Distilled-water wipe method: 7-10 days, with daily wiping and pack reloading. Larger humidors (100+ count) can take longer."
  - q: "Can you use tap water to season a humidor?"
    a: "No. Tap water minerals and chlorine damage the cedar lining and can introduce mold. Distilled water only, never tap, spring, filtered, or alkaline."
  - q: "How do I know seasoning is complete?"
    a: "Hygrometer reads steady at 70-72% RH for at least 48 hours with the lid closed. The cedar smells slightly sweet, not raw or wet. The hygrometer no longer drifts down when the lid stays sealed overnight."
published: false
---

> **Status:** Skeleton. Cluster target length is 1,500-1,800 words. Your
> first-hand seasoning experience goes here, which method you used, what
> went right, what you'd do differently.

## Why do you need to season a humidor?

[ 200-300 words. The kiln-drying problem: new Spanish cedar arrives
below 50% internal moisture and will absorb humidity aggressively
from anything inside it, including your cigars. First-hand: what
happened the time you skipped or rushed seasoning. ]

## How do you season with Boveda packs?

[ 350-450 words. Step-by-step:
1. Wipe the empty humidor with a dry cloth (no water).
2. Place 2-4 Boveda 84% RH seasoning packs (one per 25-50 cigar capacity).
3. Close the lid. Do NOT open for 14 days.
4. After 14 days, replace 84% packs with your maintenance packs (65/69/72%).
5. Add cigars only after maintenance RH stabilizes for 48 hours.

Why the 84% packs and not 72%, the cedar needs over-saturation to
load enough moisture to buffer normal use. ]

## How do you season with distilled water?

[ 350-450 words. Step-by-step:
1. Wipe the cedar lining lightly with a clean cloth dampened with
   distilled water, never wet, just damp. Avoid puddles.
2. Charge your humidification device per instructions (distilled
   water only, never tap).
3. Place the humidifier inside, close the lid, wait 24 hours.
4. Re-wipe daily for 5-7 days as the cedar absorbs.
5. Recharge the humidifier on day 3 and day 6.
6. Stop wiping once RH stabilizes at 70-72% with the lid closed
   overnight.

Why this method gets criticized: easy to over-wet, easy to introduce
mold, requires daily attention. Why some collectors still prefer it:
faster, cheaper for big humidors. ]

## How long does seasoning actually take?

[ 200-300 words. Boveda method timing: 14 days minimum, more for big
humidors. Distilled method: 7-10 days with daily attention. Why
patience matters, half-seasoned humidors are the leading cause of
new-collector cigar damage. ]

## How do you know seasoning is complete?

[ 150-250 words. The three confirmations:
1. Hygrometer steady at 70-72% RH for 48 hours.
2. Cedar smells sweet and slightly aromatic, not raw lumber.
3. RH does not drop more than 1-2% overnight with lid closed.

Common mistake: trusting an uncalibrated hygrometer. Re-do the salt
test before declaring victory. ]

## What goes wrong if you skip or rush seasoning?

[ 150-250 words. Symptoms: cigars dry within days, wrappers crack at
the foot, draws turn loose, hygrometer reads 60% even with full
humidification. The recovery: pull cigars to a sealed bag with a
72% Boveda, restart seasoning from scratch. ]

[ Internal-link block: Picking between Boveda and distilled long-term
→ see `boveda-vs-distilled-water-humidor`. Once seasoned, what RH
should you actually run → see `cigar-humidor-humidity`. Full storage
overview → see `how-to-store-cigars`. ]

---

*Stick Picks is a private catalog and journal for adult cigar collectors
aged 21+. We do not sell cigars or promote tobacco use.*
